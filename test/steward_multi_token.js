...
const CarbonCredit = artifacts.require('./ERC20PatronageReceipt_v0.sol');
...
contract('WildcardSteward owed', (accounts) => {
  ...
  //const tokenGenerationRate = 317097919838;
  const tokenGenerationRate = 1;
  ...
  beforeEach(async () => {
    ...
    carbon = await CarbonCredit.new({ from: accounts[0] });
    ...
    //function initialize(string memory name, string memory symbol, uint8 decimals, address minter) 
    await carbon.initialize("CarbonCredit", "CCT", 18, steward.address)

    // TODO: use this to make the contract address of the token deturministic: https://ethereum.stackexchange.com/a/46960/4642
    await steward.initialize(artwork.address, accounts[0], patronageDenominator, carbon.address)
    // add the tokenGenerationRateNumerator in the mint...
    await steward.listNewTokens([0, 1, 2], [accounts[0], accounts[0], accounts[0]], [patronageNumerator, patronageNumerator, patronageNumerator], [tokenGenerationRate, tokenGenerationRate, tokenGenerationRate])
  })
  ...
  it('steward: multi-token. check patronage of two tokens owed by the same patron after 10 minutes one of the tokens gets bought.', async () => {
    ...
    //await time.increase(time.duration.minutes(525600));
    await time.increase(time.duration.minutes(1000000));
    console.log("hello2")
    const creditsDue = await steward.creditsDue.call(accounts[2], 1);
    console.log(creditsDue.toString());
    ...
    const erc20stuff = await carbon.balanceOf.call(accounts[2]);
    console.log("hello")
    console.log(erc20stuff.toString());

    console.log("hello3")
    const creditsDue2 = await steward.creditsDue.call(accounts[2], 1);
    console.log(creditsDue2.toString());
